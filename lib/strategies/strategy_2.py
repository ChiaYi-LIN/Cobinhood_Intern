...
class trade_strategy_2(trade_process):
    def apply_trading_strategy(self, sar_diff, lower_shadow, upper_shadow):
        ...
        self.set_delay = 1
        self.delay = 0
        self.start_ma_stop_gain_5 = False
    ...
    def should_sell(self, i, sar_diff, upper_shadow):
        if (self.data["Close"].iloc[i] - self.exist_long[0][4])/self.exist_long[0][4] >= 0.05 or self.start_ma_stop_gain_5 == True:
            # 進階移動停利
            if self.start_ma_stop_gain_5 != True:
                self.start_ma_stop_gain_5 = True
            if self.ma_stop_gain == 0:
                ...
                if ((self.data["Close"].iloc[i] - self.exist_long[0][4])/self.exist_long[0][4]) <= self.ma_stop_gain*1/2:
                    ...
                    self.ma_stop_gain = (self.data["Close"].iloc[i] - self.exist_long[0][4])/self.exist_long[0][4] 
        # 普通停利
        elif self.data["Parabolic SAR"].iloc[i] < self.data["Low"].iloc[i]:
