...
class trade_strategy_9(trade_process):
    ...
    def should_long(self, i, sar_diff, volume_max):
        if self.data["Parabolic SAR"].iloc[i] > self.data["High"].iloc[i] and i > 14:
            if (self.data["Parabolic SAR"].iloc[i] - self.data["Low"].iloc[i])/self.data["Low"].iloc[i] > self.data["Average true range"].iloc[i]/4000 and self.data["True range"].iloc[i] > self.data["Average true range"].iloc[i]:
