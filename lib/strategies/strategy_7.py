...
class trade_strategy_7(trade_process):
    ...
    def should_long(self, i, sar_diff, volume_max):
        if self.data["Parabolic SAR"].iloc[i] > self.data["High"].iloc[i]:
            if (self.data["Parabolic SAR"].iloc[i] - self.data["Low"].iloc[i])/self.data["Low"].iloc[i] > sar_diff:
                if self.data["Volume"].iloc[i] == self.data["Volume"].iloc[i-volume_max:i+1].max():
        ...
        if self.current_situation == 1:
            ...
            if self.to_target_price:
                if self.data["Close"].iloc[i] <= weighted_cost:
                    ...
                    self.start_ma_stop_gain = False
                    self.to_target_price = False
                elif (self.data["Close"].iloc[i] - self.exist_long[0][4])/self.exist_long[0][4]  <= self.ma_stop_gain*long_2_moving_stop:
                    self.sell_signal = True
                    self.ma_stop_gain = 0
                    self.start_ma_stop_gain = False
                    self.to_target_price = False
